...
use OCA\EWS\Service\Remote\RemoteContactsService2007;
...
class ContactsService
{
    ...
    public function __construct(string                       $appName,
                                ...
                                private CardDavBackend       $LocalStore)
    {
    }
    ...
    public function configure($configuration, EWSClient $RemoteStore): void
    {
        ...
        switch ($RemoteStore->getVersion()) {
            ...
            case $RemoteStore::SERVICE_VERSION_2009:
                $this->RemoteContactsService = \OC::$server->get(RemoteContactsService2007::class);
                break;
            default:
                $this->RemoteContactsService = \OC::$server->get(RemoteContactsService::class);
                break;
        }
    ...
    function harmonizeLocalAltered($uid, $lcid, $loid, $rcid, $caid): string
    {
        ...
        if ($ci instanceof Correlation &&
            $ci->getlostate() == $lo->State) {
            // return status of action
            return $status;
        }
        // if correlation exists, try to retrieve remote object
        if ($ci instanceof Correlation && !empty($ci->getroid())) {
            try {
        ...
        if (isset($ro)) {
            ...
            elseif ($ci instanceof Correlation &&
